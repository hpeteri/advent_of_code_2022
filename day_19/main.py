...
class SimulationContext:
    def __init__(this, costs):
        ...
        this.target     = 32
        this.costs      = costs
        this.max_robots = [0,0,0,this.target]
        this.history    = "\n!!! HISTORY !!! \n"
    ...
    def make_robot(this, robot_idx):        
        for ci, cost in enumerate(this.costs[robot_idx]):
            ...
            this.history += "make {} robot for cost of {}\n".format(["ore", "clay", "obsidian", "geode"][robot_idx], sim.costs[robot_idx])
    ...
    def history_start_turn(this):
        this.history += f"=== {this.minute} ===\n"
        
    def history_resources(this):
        s = f"robots:    {this.robots}\n"
...
def simulate(sim, simulations_to_do, best_result):
    if sim.minute == sim.target - 1:
        ...
        sim.history_start_turn()
        sim.history_resources()
    ...
    for i in range(4):
        ...
        if time_for_n_robot >= 0:
            ...
            sim_0.history_start_turn()
            sim_0.history_resources()
            ...
            if best_result:
                time_left = sim_0.target - sim_0.minute

                g0 = sim_0.robots[3] * time_left
